...
2. 選擇性上傳 margin.csv 做融資風險判斷。
3. 選擇性上傳 stop_history.csv 延續既有移動停利歷史。
4. 自訂 ATR 週期與最近高點期間。
5. 依股票類型套用 ATR 倍數，並支援手動調整。
6. 計算持股總市值、投入成本、未實現損益與未實現損益率。
7. 判斷是否跌破 ATR 移動停利與融資風險是否升高。
8. 下載 atr_report.csv、atr_report.xlsx、updated_stop_history.csv。
