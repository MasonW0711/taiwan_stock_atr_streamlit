...
4. 預設自動抓取證交所與櫃買中心官方融資與法人資料，也可選擇上傳 margin.csv 覆蓋。
...
2. 盡量讓截圖包含股票代號、股票名稱、股數與損益平衡價；若來源沒有，再使用平均成本。
3. 按下「辨識持股截圖」。
4. 系統會先產生 OCR 草稿表。
...
- 若 OCR 沒抓到損益平衡價，系統會退回成本價；若兩者都沒抓到，請手動補上後再計算。

## 融資資料來源

系統預設會自動抓取官方融資與法人資料：

- 上市股票使用台灣證交所官方資料來源，抓取融資餘額與三大法人中的外資、投信買賣超。
- 上櫃股票使用櫃買中心官方資料來源，抓取融資餘額與三大法人中的外資、投信買賣超。
- 若今天尚未有資料，系統會自動往前回退到最近可用交易日。

margin.csv 仍然是選填檔案，適合以下情境：

- 你想覆蓋官方資料，改用自己整理過的欄位。
- 官方網站暫時無法抓取，你要手動補資料。
- 你想完全改用自己整理過的外資買賣超、投信買賣超或股價漲跌幅欄位。

若未上傳 margin.csv，系統會先嘗試自動抓取官方資料；只有在官方來源也抓不到時，融資風險欄位才會顯示「尚未提供融資資料」。

## 如何準備 margin.csv

若你想手動覆蓋官方資料，可準備 margin.csv。
...
若上傳 margin.csv，系統會優先使用你提供的檔案，不再使用自動抓取的官方融資與法人資料。
